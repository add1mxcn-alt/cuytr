...
local function applyShader()
    ...
    elseif selectedShader == "شادر واقعي نوعاً ما" then
...
TrollTab:AddSection({ "الكره" })

local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

local BallSelected = nil
...
local BallDropdown = TrollTab:AddDropdown({
    Name = "اختيار اللاعب",
    Default = "",
    Multi = false,
    Options = GetPlayerNames(),
    Callback = function(name)
        BallSelected = Players:FindFirstChild(name)
    end
})
...
TrollTab:AddButton({
    Name = "تحديث قائمة اللاعبين",
    Callback = function()
        BallDropdown:Set(GetPlayerNames())
    end
})
    ...
    if Target ~= Player then
        ...
        if TChar and TChar:FindFirstChild("HumanoidRootPart") and TChar:FindFirstChild("Humanoid") then
            ...
            repeat
                ...
                task.wait(1/6000)
            until not TChar or not TChar.Parent or TRoot.Velocity.Magnitude > 1000 or THum.Health <= 0 or (tick() - StartTime) > 10
...
TrollTab:AddButton({
    Name = "فلنق بالكره",
    Callback = function()
        if not BallSelected then return end
        FlingBall(BallSelected)
    end
})
...
local function GetRandomBallTarget()
    local Available = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p ~= LastRandomBall and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            local Hum = p.Character:FindFirstChildOfClass("Humanoid")
            if Hum and Hum.Health > 0 then
                table.insert(Available, p)
            end
    ...
    if #Available == 0 then
        LastRandomBall = nil
        return GetRandomBallTarget()
    end
    local Chosen = Available[math.random(1, #Available)]
    LastRandomBall = Chosen
    return Chosen
end

TrollTab:AddButton({
    Name = "فلنق عشوائي بالكره",
    Callback = function()
        local Target = GetRandomBallTarget()
        if Target then
            FlingBall(Target)
        end
    end
